...
const MORNING_SYSTEM_PROMPT = `You write deeply intimate, hyper-personal morning identity audio scripts in SECOND PERSON ("you"). Every sentence must feel like it was written by someone who has lived inside this person's mind and is speaking directly to them.

CRITICAL PERSONALIZATION RULES:
- Write ENTIRELY in second person: "you wake up", "you feel", "you move". NEVER use their name — address them only as "you" throughout.
...
- Use ONLY present-tense second person language: "you are", "you choose", "you move", "you receive"
...
const NIGHT_SYSTEM_PROMPT = `You write deeply intimate nighttime identity regulation scripts for sleep conditioning in SECOND PERSON ("you"). This should feel like the most personal, tender thing anyone has ever said to this person.

CRITICAL PERSONALIZATION RULES:
- Write ENTIRELY in second person: "you release", "you rest", "you are safe". NEVER use their name — address them only as "you" throughout.
...
STRICT LANGUAGE RULES:
- Use ONLY present-tense second person language: "you are", "you release", "you rest", "you trust", "you are safe"
...
export async function generateIdentityScript(
  ...
  const userMessage =
    type === "morning"
      ? `Write a morning identity audio script for this person:\n\n${userContext}\n\nThis is NOT a generic script. Write entirely in second person ("you") — NEVER use their name. Extract specific imagery from their identity statement and dream life description. Build sensory scenes from THEIR specific world — their workspace, their morning routine, their environment. Reference their own words back to them. Every sentence should feel like it was written after spending a week inside their life. If they mention specific activities, places, or feelings in their dream life — build scenes around those EXACT details. Do NOT start with "Close your eyes" or any generic opener. First line must drop them into a vivid sensory scene from THEIR specific life.`
      : `Write a nighttime identity regulation script for this person:\n\n${userContext}\n\nThis plays as they fall asleep. Write entirely in second person ("you") — NEVER use their name. Paint quiet, intimate scenes from THEIR specific dream life — the evening version. Reference the specific work they put down, the specific identity they carry into rest. Use their own language from their identity statement, woven gently into scenes of safety and peace. Build sensory details from THEIR world — not generic beaches or forests unless they specifically mentioned those. Every paragraph must contain something that could only apply to THIS person. Do NOT start with generic instructions. First line should place them in a specific moment of their actual life settling into rest.`;
